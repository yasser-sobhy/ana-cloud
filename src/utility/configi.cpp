...
void Configi::read(QString file)
{
    ...
    else qDebug() << "Configi: couldn't open ini file" << file;
...
QVariant Configi::get(QString section, QString key, QString defaultValue){
    ...
    if(sction)
    {
        ...
        else return QVariant(defaultValue);
    }
...
void Configi::close()
{
    ...
    qDeleteAll(m_sections.values());
    m_sections.clear();

    ongoingSection = nullptr;

    m_error = ConfigiError();
    currentLine = 0;
}
