...
QByteArray HttpRequest::setPath(QString newPath) {
    return path = QByteArray().append(newPath);
}
...
int HttpRequest::getParameterLength() const {
    return parameters.keys().length();
}

QByteArray HttpRequest::getTokenValue(QByteArray token){
    return pathTokens->value(token);
}

QHash<QByteArray, QByteArray> *HttpRequest::getTokens(){
    return pathTokens;
}

void HttpRequest::setTokens(QHash<QByteArray, QByteArray> *tokens){
    pathTokens = tokens;
}
...
QTemporaryFile* HttpRequest::getUploadedFile(const QByteArray fieldName) {
    ...
    qDebug() << "uploadedFiles:";
    for(QString d : uploadedFiles.keys()) qDebug() << d;
